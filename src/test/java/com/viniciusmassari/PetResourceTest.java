...
public class PetResourceTest {
    ...
    public void create_pet(){
     ...
     var response =   given().body(new LoginOrganizationRequestDTO("johndoe@email.com", "12345678")).contentType(ContentType.JSON).post("http://localhost:8080/organization/login").then().extract().body();
        ...
        given().body(new CreatePetRequestDTO("Spike","special dog", (short) 10, LivingSpace.SMALL, PetIndependence.MEDIUM, PetEnergy.LOW, PetSize.MEDIUM,"some requirements")).contentType(ContentType.JSON).header(new Header("Authorization","Bearer " + response)).post("http://localhost:8080/pet/").then().statusCode(201);
